...
func (r *WAG019) Check(fset *token.FileSet, file *ast.File, path string) []LintIssue {
	...
	// Collect all jobs and their dependencies
	ast.Inspect(file, func(n ast.Node) bool {
		genDecl, ok := n.(*ast.GenDecl)
		if !ok {
			return true
		...
		for _, spec := range genDecl.Specs {
			...
			for i, name := range valueSpec.Names {
				...
				for _, elt := range lit.Elts {
					...
					// Extract dependency names using shared helper
					deps := extractDependencyNames(kv.Value)
					jobDeps[name.Name] = deps
				}
			}
	...
	var detectCycle func(job string, path []string) []string
	detectCycle = func(job string, currentPath []string) []string {
		...
		for _, dep := range jobDeps[job] {
			// Check if dependency is in the current recursion stack (cycle)
			if recStack[dep] {
		...
		if !visited[job] {
			if cycle := detectCycle(job, nil); cycle != nil {
				...
				if !reportedCycles[cycleKey] {
					...
					// Report the cycle at the first job's position
...
// It rotates the cycle so the lexicographically smallest job is first.
func normalizeCycle(cycle []string) string {
	if len(cycle) == 0 {
		return ""
	}

	// Find the lexicographically smallest job
	minIdx := 0
